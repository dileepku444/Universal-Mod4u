"""Raylogic MOD4U relay/switch platform.
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    entities = [
        RaylogicMod4uSwitch(hass, entry, device, ch_num, state)
        for ch_num, state in device.channel_states.items()
        if state.get("type") == CH_TYPE_RELAY
    ]
    if entities:
        _LOGGER.info("Setting up %d MOD4U relay channel(s) on %s", len(entities), device.ip)
        async_add_entities(entities)
    else:
        _LOGGER.info(
            "Raylogic MOD4U %s: koi channel abhi tak nahi seekha gaya "
    ...
    def _on_new_channel(ch_num, state):
        async_add_entities([RaylogicMod4uSwitch(hass, entry, device, ch_num, state)])

    device.new_channel_callback = _on_new_channel


class RaylogicMod4uSwitch(SwitchEntity):
    ...
    def __init__(self, hass, entry, device: RaylogicMod4uDevice, ch_num, initial_state):
        ...
        self._attr_name = f"mod4u_{suffix}_area{area}_ch{ch_num}"
