...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    entities = [
        RaylogicMod4uFan(hass, entry, device, ch_num, state)
        for ch_num, state in device.channel_states.items()
        if state.get("type") == CH_TYPE_FAN
    ]
    if entities:
        _LOGGER.info("Setting up %d MOD4U fan channel(s) on %s", len(entities), device.ip)
        async_add_entities(entities)


class RaylogicMod4uFan(FanEntity):
    ...
    def __init__(self, hass, entry, device: RaylogicMod4uDevice, ch_num, initial_state):
        ...
        self._attr_name = f"mod4u_{suffix}_area{area}_ch{ch_num}_fan"
